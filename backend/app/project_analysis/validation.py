...
def prevalidate_virtual_files(root: str | Path, index: dict[str, Any], contract: dict[str, Any]) -> dict[str, Any]:
    ...
    checks.extend(["changed imports", "deleted references", "renamed known callers"])
    return {"status": "passed", "checks": checks, "warnings": _warnings(parsed), "validated_files": sorted(seen), "validated_bytes": total}
