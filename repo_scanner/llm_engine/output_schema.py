...
class RepoRisk(BaseModel):
    ...
    evidence: str = Field(..., description="Evidence from scanner, graph, or analysis output.")
...
class RecommendedAction(BaseModel):
    ...
    target_area: str = Field(..., description="File, folder, module, or subsystem to inspect/change.")
    requires_file_edit: bool = Field(..., description="Whether this action requires modifying files.")
    rationale: str = Field(..., description="Why this action matters.")


class RepoDecision(BaseModel):
    repo_identity: str = Field(..., description="Short classification of the repository.")
    architecture_summary: str = Field(..., description="Short description of major architecture.")
